...
function Demo() {
    ...
    // Effect to update filtered questions when activeCategory changes
    useEffect(() => {
        ...
        setCurrentQuestionIndex(0); // Reset to the first question of the new filter
        setShowAnswer(false); // Hide answer when filter changes
    }, [activeCategory]);
            ...
            <div className={styles.container}>
                <div className={styles.card}>
                    <header className={styles.header}>
                        <h1 className={styles.title}>분야별 면접 연습</h1>
                        <div className={styles.categoryFilters}>
                            {['all', '프론트엔드', '백엔드'].map(category => (
                                <button
                                    key={category}
                ...
                <p className={styles.demoText}>Gemini가 생성한 데모 페이지입니다.</p>
            </div>
        </div>
    );
}
